...
class Otkp extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('otkp/index', $data);
    ...
    public function editProfile()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('wrapper/sidebar', $data);
            $this->load->view('wrapper/topbar', $data);
            $this->load->view('otkp/edit-profile', $data);
            $this->load->view('wrapper/footer');
        } else {
    ...
    public function IDCard($nis)
    {
        ...
        $this->load->view('wrapper/sidebar', $data);
        $this->load->view('wrapper/topbar', $data);
        $this->load->view('otkp/id-card', $data);
        $this->load->view('wrapper/footer');
        ...
        $html = $this->load->view('otkp/id-card', [], true);
        $mpdf->WriteHTML($html);
    ...
    public function editdata($nis)
    {
        ...
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['siswa'] = $this->db->get_where('master', ['nis' => $nis])->row_array();
        $data['data'] = $this->db->get_where('tbl_iduka', ['jurusan' => 'OTKP'])->result_array();
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('wrapper/sidebar', $data);
            $this->load->view('wrapper/topbar', $data);
            $this->load->view('otkp/edit-data', $data);
            $this->load->view('wrapper/footer');
        } else {
            $data = [
                'name' => $this->input->post('name'),
